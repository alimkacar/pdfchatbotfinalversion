"""
PDF RAG Chatbot - Arama Motoru Sınıfı
...
class SearchEngine:
    ...
    def index_document(self, document: ProcessedDocument):
        """
        Dökümanı indeksler (TF-IDF oluşturur)
        
        Args:
            document: İşlenmiş döküman
    ...
    def search(self, query: str, max_results: int = 5, min_similarity: float = 0.01) -> SearchResponse:
        ...
        Returns:
            SearchResponse: Arama sonuçları
        """
    ...
    def get_similar_chunks(self, chunk_id: int, max_results: int = 3) -> list:
        """
        Belirli bir chunk'a benzer chunk'ları bulur
        
        Args:
            chunk_id: Referans chunk ID'si
            max_results: Maksimum sonuç sayısı
            
        Returns:
            Benzer chunk'lar listesi
        """
        try:
            if not self.current_document or chunk_id >= len(self.current_document.chunks):
                return []
            
            # Referans chunk'ın vektörü
            ...
            # Kendisi hariç en benzer chunk'ları al
            similarities[chunk_id] = -1  # Kendisini hariç tut
            top_indices = similarities.argsort()[-max_results:][::-1]
    ...
    def get_top_terms(self, n: int = 10) -> list:
        """
        En önemli terimleri döndürür
        
        Args:
            n: Döndürülecek terim sayısı
            
        Returns:
            En önemli terimler listesi
        """
        try:
